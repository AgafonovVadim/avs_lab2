...
2. На `gateway-vm` и `client-vm` пропишите статические адреса для внутренней сети в `interfaces` (ISU - считается, как ваш ISU % 100):
...
3. На `gateway-vm` оставьте внешний интерфейс в режиме DHCP (NAT VirtualBox).
4. Проверьте связность: `ping 10.10.ISU.2` с `gateway-vm`, затем `ping 8.8.8.8` с `gateway-vm`.
5. Включите форвардинг в ядре на `gateway-vm`:
   ```bash
